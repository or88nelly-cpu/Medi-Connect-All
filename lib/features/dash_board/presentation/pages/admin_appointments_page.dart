...
class _AdminAppointmentsPageState extends State<AdminAppointmentsPage> {
  ...
  Future<void> _selectDate(BuildContext context, DateTime currentDate, Function(DateTime) onChanged) async {
                    ...
                    return BlocBuilder<AdminAppointmentsFilterCubit, AdminAppointmentsFilterState>(
                      builder: (context, filterState) {
                        ...
                        final targetDateApts = appointments
                            .where((a) => _isSameDay(a.appointmentDate, filterState.selectedDate))
                            .toList();
                        ...
                        final filteredList = targetDateApts.where((apt) {
                          final matchesStatus = filterState.filterStatus == 'All' ||
                              apt.status.toLowerCase() == filterState.filterStatus.toLowerCase();

                          final matchesSearch = apt.patientName.toLowerCase().contains(filterState.searchQuery.toLowerCase()) ||
                              apt.doctorName.toLowerCase().contains(filterState.searchQuery.toLowerCase()) ||
                              apt.specialty.toLowerCase().contains(filterState.searchQuery.toLowerCase());

                          return matchesStatus && matchesSearch;
                        }).toList();

                        // Sort by chronological time
                        filteredList.sort((a, b) => _compareTimes(a.appointmentTime, b.appointmentTime));

                        return SingleChildScrollView(
                          physics: const BouncingScrollPhysics(),
                          padding: EdgeInsets.symmetric(horizontal: 20.r, vertical: 16.r).copyWith(top: 0.h),
                          child: Column(
                            ...
                            children: [
                              ...
                              InkWell(
                                onTap: () => _selectDate(
                                  ...
                                  (newDate) => context.read<AdminAppointmentsFilterCubit>().changeSelectedDate(newDate),
                                ),
                                ...
                                child: Padding(
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      ...
                                      SizedBox(width: 8.w),
                                      Text(
                                        DateFormat('dd MMMM yyyy, EEEE').format(filterState.selectedDate),
                                        style: AppTextStyles.bodyMedium.copyWith(
                                ...
                                children: [
                                  Expanded(
                                    child: Container(
                                      ...
                                      child: TextField(
                                        onChanged: (val) => context.read<AdminAppointmentsFilterCubit>().changeSearchQuery(val),
                                        style: TextStyle(
                                          color: isDark ? Colors.white : AppColors.textPrimary,
                                          fontSize: 13.sp,
                                        ),
                              ...
                              SingleChildScrollView(
                                ...
                                child: Row(
                                  children: ['All', 'Confirmed', 'Pending', 'Completed', 'Cancelled'].map((status) {
                                    final isSelected = filterState.filterStatus == status;
                                    return Padding(
                              ...
                              Text(
                                _isSameDay(filterState.selectedDate, DateTime.now())
                                    ? "Today, ${DateFormat('dd MMMM yyyy').format(filterState.selectedDate)}"
                                    : DateFormat('dd MMMM yyyy').format(filterState.selectedDate),
                                style: AppTextStyles.bodyMedium.copyWith(
                                  fontWeight: FontWeight.bold,
                                  color: isDark ? Colors.white : AppColors.textPrimary,
                                  fontSize: 14.sp,
