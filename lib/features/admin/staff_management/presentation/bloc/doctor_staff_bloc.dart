...
import 'package:medi_connect/features/admin/staff_management/domain/repositories/doctor_staff_repository.dart';
...
class DoctorStaffBloc extends Bloc<DoctorStaffEvent, DoctorStaffState> {
  ...
  Future<void> _onLoadDoctorStaff(
    LoadDoctorStaff event,
    Emitter<DoctorStaffState> emit,
  ) async {
  ...
  Future<void> _onCreateDoctorStaffMember(
    CreateDoctorStaffMember event,
    Emitter<DoctorStaffState> emit,
  ) async {
    emit(DoctorStaffLoading());
    ...
    result.fold((failure) => emit(DoctorStaffError(failure.message)), (user) {
      ...
      add(LoadDoctorStaff('All'));
    });
  }

  Future<void> _onUpdateDoctorStaffMember(
    UpdateDoctorStaffMember event,
    Emitter<DoctorStaffState> emit,
  ) async {
    emit(DoctorStaffLoading());
  ...
  Future<void> _onDeleteDoctorStaffMember(
    DeleteDoctorStaffMember event,
    Emitter<DoctorStaffState> emit,
  ) async {
    emit(DoctorStaffLoading());
    final result = await _repository.deleteDoctorStaffMember(event.userId);
    result.fold((failure) => emit(DoctorStaffError(failure.message)), (_) {
      emit(DoctorStaffActionSuccess());
