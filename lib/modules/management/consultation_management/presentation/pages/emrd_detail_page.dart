...
class _EmrdDetailPageState extends State<EmrdDetailPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      ...
      child: BlocBuilder<EmrdBloc, EmrdState>(
        builder: (context, state) {
          EmrdView currentView = EmrdView.dashboard;
          if (state is EmrdLoaded) {
            currentView = state.currentView;
          }

          return CustomScaffold(
            customAppbar: currentView == EmrdView.medicalRecordManagement
                ? CommonAppBar(
                    title: "Medical Record Management",
                    leading: IconButton(
                      icon: const Icon(Icons.arrow_back),
                      onPressed: () {
                        context.read<EmrdBloc>().add(const SwitchEmrdView(EmrdView.dashboard));
                      },
                    ),
                  )
                : currentView == EmrdView.patientRegistry
                    ? CommonAppBar(
                    ...
                    : const CommonAppBar(title: "EMRD Operations"),
            body: Builder(
              builder: (context) {
                if (state is EmrdLoading) {
                  ...
                  if (state.currentView == EmrdView.medicalRecordManagement) {
                    final filteredRecords = emrRecords
                        .where((r) => r['specialty'] != 'Customer Care')
                        .toList();

                    // --- ORIGINAL EMR RECORDS LIST VIEW VIEWPORT ---
                  ...
                  if (state.currentView == EmrdView.patientRegistry) {
                    final registryRecords = emrRecords
                        .where((r) => r['specialty'] == 'Customer Care')
                        .toList();
                                  ...
                                  GridView.count(
                                    ...
                                    children: [
                                      EmrdOperationCard(
                                        ...
                                        isDark: isDark,
                                        onTap: () {
                                          context.read<EmrdBloc>().add(const SwitchEmrdView(EmrdView.patientRegistry));
                                        },
                                      ),
                                      EmrdOperationCard(
                                        title: "Medical Record Management",
                                        ...
                                        isDark: isDark,
                                        onTap: () {
                                          context.read<EmrdBloc>().add(const SwitchEmrdView(EmrdView.medicalRecordManagement));
                                        },
                                      ),
                                      EmrdOperationCard(
